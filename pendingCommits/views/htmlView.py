...
class HtmlView:
	def __init__(self, options):
		...
		<body class="b9 f9" style="background:#424242;color:#e7e7e7;font-family:monospace;">'''
	...
	def addLogLine(self, message, revisions, author, paths):
		...
		if self.options.showPaths is True:
			for path in paths: 
				...
				if path.action == 'M':
					print os.popen("svn diff " + self.rootDir + path.path.replace('/trunk/', '') + " -r " + int(revisions) + "   | ./ansiToHtml ").read()
			print "<br/>"
